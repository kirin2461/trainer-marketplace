...
- Reviews
- Recommendations
- Marketplace: listings, filters, escrow buy/confirm/cancel, seller reviews
- Static pages
"""
...
class TestMarket:
    """Tests for the gear marketplace (Lolz-style lots + escrow)."""
    ...
    def test_buy_creates_escrow(self, client: TestClient, test_client_user, test_product, db: Session) -> None:
        _authenticate_client(client, test_client_user.id)
        ...
        assert order.status == "escrow"
        assert float(order.fee) == 7.00  # 7% of 100
    ...
    def _make_escrow_order(self, client, test_client_user, test_product, db):
        _authenticate_client(client, test_client_user.id)
        response = client.post(f"/api/market/products/{test_product.id}/buy")
        assert response.status_code == 200
        return response.json()["order_id"]

    def test_confirm_order_releases_funds(self, client: TestClient, test_client_user, test_trainer, test_product, db: Session) -> None:
        order_id = self._make_escrow_order(client, test_client_user, test_product, db)
        seller_before = float(test_trainer.wallet_balance)

        response = client.post(f"/api/market/orders/{order_id}/confirm")
        assert response.status_code == 200
        assert response.json()["seller_amount"] == 93.00

        db.refresh(test_trainer)
        assert float(test_trainer.wallet_balance) == seller_before + 93.00
    ...
    def test_cancel_order_refunds(self, client: TestClient, test_client_user, test_product, db: Session) -> None:
        order_id = self._make_escrow_order(client, test_client_user, test_product, db)
        balance_after_buy = float(test_client_user.wallet_balance)

        response = client.post(f"/api/market/orders/{order_id}/cancel")
        assert response.status_code == 200
        assert response.json()["refunded"] == 100.00

        db.refresh(test_client_user)
        db.refresh(test_product)
        assert float(test_client_user.wallet_balance) == balance_after_buy + 100.00
        assert test_product.status == "active"  # lot is back on sale

    def test_seller_review_flow(self, client: TestClient, test_client_user, test_trainer, test_product, db: Session) -> None:
        order_id = self._make_escrow_order(client, test_client_user, test_product, db)
        client.post(f"/api/market/orders/{order_id}/confirm")
    ...
    def test_review_before_complete_rejected(self, client: TestClient, test_client_user, test_product, db: Session) -> None:
        order_id = self._make_escrow_order(client, test_client_user, test_product, db)
        response = client.post(f"/api/market/orders/{order_id}/review", data={"rating": 5})
        assert response.status_code == 400
...
class TestPages:
    ...
    def test_about(self, client: TestClient) -> None:
        ...
        assert response.status_code == 200
        assert "Безопасная сделка" in response.text
