...
def _authenticate_client(client: TestClient, user_id: int) -> None:
    """Set a signed session cookie directly on the TestClient."""
    client.cookies.set("user_id", app_main.make_session_cookie(user_id))
...
def test_trainer(db):
    ...
    trainer = User(
        ...
        showcase_until=datetime.utcnow() + timedelta(days=30),  # paid placement active
    )
    db.add(trainer)
    db.commit()
    db.refresh(trainer)
