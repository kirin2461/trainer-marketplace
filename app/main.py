...
from app.database import (
    ...
    ClimbingGym, trainer_gyms, SessionLocal,
)
...
from app.gyms_data import (
    seed_climbing_gyms, link_demo_trainers,
    get_trainer_gyms, set_trainer_gyms, get_gym_trainers,
)
...
# Yandex Maps JS API key (optional — the map works without it, but a key
# removes usage limits). Set via environment variable YANDEX_MAPS_API_KEY.
...
# Seed climbing gyms (скалодромы) on startup — idempotent
try:
    _seed_db = SessionLocal()
    try:
        seed_climbing_gyms(_seed_db)
    finally:
        _seed_db.close()
except SQLAlchemyError:
    pass

templates.env.globals["YANDEX_MAPS_API_KEY"] = YANDEX_MAPS_API_KEY
...
async def trainer_detail(trainer_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("trainer_detail.html", {
        ...
        "trainer_gyms": get_trainer_gyms(db, trainer_id),
    })
...
async def dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    if user.role == "trainer":
        ...
        return templates.TemplateResponse("dashboard_trainer.html", {
            ...
            "gyms_map": _bookings_gyms_map(db, [b for b, _ in bookings]),
        })
    else:
...
def _bookings_gyms_map(db: Session, bookings: List[Booking]) -> Dict[int, ClimbingGym]:
    """Map gym_id -> ClimbingGym for a list of bookings (for dashboards)."""
    gym_ids = {b.gym_id for b in bookings if b.gym_id}
    if not gym_ids:
        return {}
    return {g.id: g for g in db.query(ClimbingGym).filter(ClimbingGym.id.in_(gym_ids)).all()}
...
async def showcase_page(request: Request, db: Session = Depends(get_db)):
    ...
    all_gyms = db.query(ClimbingGym).order_by(ClimbingGym.city, ClimbingGym.name).all()
    return templates.TemplateResponse("showcase.html", {
        "request": request, "user": user, "all_gyms": all_gyms,
        "trainer_gym_ids": {g.id for g in get_trainer_gyms(db, user.id)},
    })
...
async def book_page(trainer_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if not trainer:
        raise HTTPException(status_code=404, detail="Trainer not found")
    trainer_gym_list = get_trainer_gyms(db, trainer_id)
    all_gyms = db.query(ClimbingGym).order_by(ClimbingGym.city, ClimbingGym.name).all()
    return templates.TemplateResponse("book.html", {
        "request": request, "user": user, "trainer": trainer,
        "trainer_gyms": trainer_gym_list, "all_gyms": all_gyms,
    })
...
# ============================================================
# CLIMBING GYMS (СКАЛОДРОМЫ) — pages & API
# ============================================================

def _gym_to_dict(gym: ClimbingGym, trainers_count: int = 0) -> dict:
    return {
        "id": gym.id, "name": gym.name, "city": gym.city,
        "address": gym.address, "metro": gym.metro or "",
        "lat": gym.lat, "lng": gym.lng,
        "disciplines": gym.disciplines or [],
        "disc_names": ", ".join(discipline_name(d) for d in (gym.disciplines or [])),
        "wall_height": gym.wall_height or "", "price_from": gym.price_from or 0,
        "description": gym.description or "", "website": gym.website or "",
        "trainers_count": trainers_count,
    }

@app.get("/gyms", response_class=HTMLResponse)
async def gyms_page(request: Request, city: str = None, db: Session = Depends(get_db)):
    """Climbing gyms catalog with an interactive Yandex map."""
    user = get_current_user(request, db)
    query = db.query(ClimbingGym)
    if city:
        query = query.filter(ClimbingGym.city == city)
    gyms = query.order_by(ClimbingGym.city, ClimbingGym.name).all()

    counts = dict(
        db.query(trainer_gyms.c.gym_id, func.count(trainer_gyms.c.trainer_id))
        .group_by(trainer_gyms.c.gym_id).all()
    )
    gyms_dicts = [_gym_to_dict(g, counts.get(g.id, 0)) for g in gyms]
    gym_cities = [c[0] for c in db.query(ClimbingGym.city).distinct().order_by(ClimbingGym.city).all()]

    return templates.TemplateResponse("gyms.html", {
        "request": request, "user": user, "gyms": gyms,
        "gyms_dicts": gyms_dicts, "gym_cities": gym_cities,
        "current_city": city or "", "total_gyms": db.query(ClimbingGym).count(),
    })
...
async def gym_detail_page(gym_id: int, request: Request, db: Session = Depends(get_db)):
    """Climbing gym detail: info, map pin and attached trainers."""
    user = get_current_user(request, db)
    gym = db.query(ClimbingGym).filter(ClimbingGym.id == gym_id).first()
    if not gym:
        raise HTTPException(status_code=404, detail="Climbing gym not found")
    trainers = get_gym_trainers(db, gym_id)
    return templates.TemplateResponse("gym_detail.html", {
        "request": request, "user": user, "gym": gym, "gym_trainers": trainers,
    })

@app.get("/api/gyms")
async def api_gyms(city: str = None, db: Session = Depends(get_db)):
    """JSON list of climbing gyms (used by the map and external consumers)."""
    query = db.query(ClimbingGym)
    if city:
        query = query.filter(ClimbingGym.city == city)
    gyms = query.order_by(ClimbingGym.city, ClimbingGym.name).all()
    ...
    return [_gym_to_dict(g, counts.get(g.id, 0)) for g in gyms]
...
async def api_update_showcase(
    ...
    disciplines: List[str] = Form([]), work_formats: List[str] = Form([]),
    student_levels: List[str] = Form([]), gym_ids: List[int] = Form([]),
    db: Session = Depends(get_db),
):
    ...
    # Validate gym attachments (trainer's home climbing gyms)
    valid_gym_ids = {g.id for g in db.query(ClimbingGym).filter(ClimbingGym.id.in_(gym_ids or [])).all()}
    unknown_gyms = set(gym_ids or []) - valid_gym_ids
    if unknown_gyms:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Unknown climbing gym id")
    try:
        ...
        # Attach trainer to the chosen climbing gyms
        set_trainer_gyms(db, user.id, sorted(valid_gym_ids))
...
async def api_create_booking(
    ...
    # Optional climbing gym chosen for the session ("" = договориться в чате)
    gym = None
    if gym_id and gym_id.strip():
        try:
            gym_id_int = int(gym_id)
        except ValueError:
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Invalid gym id")
        gym = db.query(ClimbingGym).filter(ClimbingGym.id == gym_id_int).first()
        if not gym:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Climbing gym not found")
    ...
    try:
        ...
        booking_desc = f"Booking with {trainer.name}"
        if gym:
            booking_desc += f" @ {gym.name}"
        db.add(WalletTransaction(
            user_id=user.id, type="payment", amount=-amount,
            balance_after=user.wallet_balance, description=booking_desc,
