...
from sqlalchemy import create_engine, Column, Integer, String, Float, DateTime, Text, Boolean, ForeignKey, JSON, Numeric, Table
...
class Booking(Base):
    ...
    created_at = Column(DateTime, default=datetime.utcnow)
    notes = Column(Text)
    gym_id = Column(Integer, ForeignKey("climbing_gyms.id"), nullable=True)  # chosen climbing gym

# ============================================================
# CLIMBING GYMS (скалодромы)
# ============================================================

# Many-to-many: trainers <-> climbing gyms they are attached to
trainer_gyms = Table(
    "trainer_gyms", Base.metadata,
    Column("trainer_id", Integer, ForeignKey("users.id"), primary_key=True),
    Column("gym_id", Integer, ForeignKey("climbing_gyms.id"), primary_key=True),
)

class ClimbingGym(Base):
    """A climbing gym / wall (скалодром) shown on the Yandex map."""
    __tablename__ = "climbing_gyms"
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(120), nullable=False)
    city = Column(String(100), nullable=False, index=True)
    address = Column(String(200), nullable=False)
    metro = Column(String(80))                 # nearest metro / landmark
    lat = Column(Float, nullable=False)
    lng = Column(Float, nullable=False)
    disciplines = Column(JSON, default=list)   # ["bouldering", "lead", "speed", ...]
    wall_height = Column(String(40))           # e.g. "12 м"
    price_from = Column(Integer, default=0)    # entry price from, RUB
...
def migrate_db():
    ...
    with engine.connect() as conn:
        ...
        # bookings.gym_id — chosen climbing gym for the session
        if "bookings" in insp.get_table_names():
            bookings_cols = {c["name"] for c in insp.get_columns("bookings")}
            if "gym_id" not in bookings_cols:
                conn.execute(text("ALTER TABLE bookings ADD COLUMN gym_id INTEGER"))
        conn.commit()
